...
|---------|------|---------|
| `cachoujs` | repo root | 0.5.1 |
| `@cachoujs/compiler` | `packages/compiler` | 0.5.1 |
| `@cachoujs/create` | `packages/create-cachou` | 0.5.1 |
...
Do **not** jump versions (e.g. 0.4.0 → 0.5.1) for routine work. Prefer many small patch releases over large batches.
...
Update root [CHANGELOG.md](../CHANGELOG.md) **first**, under a new version heading (e.g. `## 0.5.1`):
...
npm **forbids** republishing the same version. Always bump (next: **0.5.2**):
