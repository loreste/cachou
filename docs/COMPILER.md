...
- Component CSS is loaded dynamically only in browser contexts, so generated components remain importable by raw Node SSR; the sibling `.css` artifact is still emitted for Vite/browser builds.
