...
**Current line:** 0.6.x (patch-first; current **0.6.1**). Public APIs may still change before 1.0.
...
- Control-flow helpers (`Show` / `For` / `Switch`) are unwrapped correctly on SSR and as mount roots (0.6.1+).
- Deployment conventions are intentionally minimal — see [DEPLOY.md](./DEPLOY.md). **Node SSR** is the primary supported recipe; **Fetch adapters** (`cachoujs/ssr-adapters`) cover Workers/Deno/Bun as a candidate surface.
