...
const resultsPath = process.env.CACHOU_COMPARE_RESULTS_PATH || join(tmpdir(), `cachou-compare-results-${port}.json`);
const reportPath = process.env.CACHOU_COMPARE_REPORT_PATH || "";
const historyPath = process.env.CACHOU_COMPARE_HISTORY_PATH || (reportPath ? join(dirname(reportPath), "history.json") : "");
...
async function runSafariComparison() {
  ...
  return {
    ...
    failures: results.failures || (parts[4]
      ? parts[4].split(",").filter(Boolean).map(safeDecode)
      : []),
...
function printStandings(summary) {
  ...
  for (const [scenario, items] of byScenario) {
    ...
    const line = ranked
      .map((item, index) => {
        const stats = item.stats ? ` p95=${item.stats.p95.toFixed(2)} σ=${item.stats.stddev.toFixed(2)}` : "";
        return `${index + 1}. ${item.adapter} ${item.duration.toFixed(2)}ms${stats}`;
      })
      .join(" | ");
    console.log(`${scenario}: CachouJS rank ${cachouRank}/${ranked.length}; fastest ${fastest.adapter} ${fastest.duration.toFixed(2)}ms`);
    console.log(`  ${line}`);
  }
}
...
try {
  ...
  await waitForServer();
  const summary = await runSafariComparison();
  const total = summary.adapters * summary.scenarios;
  console.log(`Competitive benchmarks: ${total - summary.failed}/${total} completed`);
  ...
  if (reportPath) {
    ...
    await writeFile(join(dirname(reportPath), `${generatedAt.replace(/[:.]/g, "-")}-${basename(reportPath)}`), JSON.stringify(report, null, 2));
    if (historyPath) {
